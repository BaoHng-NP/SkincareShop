...
namespace SkincareProductSalesSystem.Pages.Customer
{
    public class RecommendedModel : PageModel
    {
        ...
        public RecommendedModel(ISkinTypeService skinTypeService, IProductService productService, IAccountService accountService)
        {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                if (!string.IsNullOrEmpty(userIdString) && int.TryParse(userIdString, out int userId))
                {
                    ...
                    if (user != null)
                    {
                        if (SkinTypeId == 0 && user.SkinTypeId.HasValue)
                        {
                            SkinTypeId = user.SkinTypeId.Value;
                        }
                        else if (SkinTypeId == 0 && !user.SkinTypeId.HasValue)
                        {
                            return RedirectToPage("/Customer/SkinTest");
                        }
                    }
                }
            }
